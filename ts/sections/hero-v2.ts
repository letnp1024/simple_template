...
export function initHeroV2() {
  const hero = document.querySelector('.hero-v2') as HTMLElement | null;
  if (!hero) return;

  const overlay = hero.querySelector('.hero-v2__overlay') as HTMLElement;
  const contentItems = hero.querySelectorAll('.hero-v2__content') as NodeListOf<HTMLElement>;
  const paginationDots = hero.querySelectorAll('.hero-v2__pagination-dot') as NodeListOf<HTMLElement>;

  if (!overlay || contentItems.length === 0) return;
  ...
  function updateSlide(index: number) {
    ...
    currentIndex = index;
    const position: Position = positions[index % positions.length];
