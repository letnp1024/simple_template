  ...
  const animateCardTransition = (newIndex: number, direction: 'next' | 'prev' | 'direct') => {
    ...
    if (isMobile) {
      ...
      targetStates.forEach(({ card, transform, opacity, width }) => {
        const cardLeft = card.querySelector('.service-card__left') as HTMLElement;
        const cardRight = card.querySelector('.service-card__right') as HTMLElement;
        ...
        if (cardLeft) {
          tl.to(
            ...
            0, // Start at the same time as card animation
          );
        }
