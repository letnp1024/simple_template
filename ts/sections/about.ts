...
export function initAboutSection() {
  const aboutSection = document.querySelector('#about-us') as HTMLElement | null;
  ...
  // Animate slide out
  const animateSlideOut = (slide: HTMLElement) => {
    const image = slide.querySelector(
      '.about-image img',
    ) as HTMLImageElement | null;
  ...
  const handleParallax = () => {
    ...
    if (!image || !activeSlide.classList.contains('active')) return;
