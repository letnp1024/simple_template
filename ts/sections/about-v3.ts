...
export function initAboutV3() {
  ...
  const barItems = aboutSection.querySelectorAll('.about-v3__bar-item') as NodeListOf<HTMLElement>;
  const cardContents = aboutSection.querySelectorAll('.about-v3__card-content') as NodeListOf<HTMLElement>;

  if (barItems.length === 0 || cardContents.length === 0) return;

  let currentIndex = 0;

  // Update content based on selected bar item
  function updateContent(index: number) {
    if (index < 0 || index >= barItems.length) return;
