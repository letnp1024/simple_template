...
export function initHeroV3() {
  const hero = document.querySelector('.hero-v3') as HTMLElement | null;
  if (!hero) return;

  const bgContainer = hero.querySelector('.hero-v3__bg-container') as HTMLElement;
  const bgImages = hero.querySelectorAll('.hero-v3__bg') as NodeListOf<HTMLElement>;

  if (!bgContainer || bgImages.length === 0) return;
  ...
  function updateBackground(index: number) {
  ...
  function goToBackground(index: number) {
